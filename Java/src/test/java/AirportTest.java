...
public class AirportTest {
    ...
    public void testGetTransportMilitaryPlanes() {
        ...
        Assert.assertFalse(transportMilitaryPlanes.isEmpty(), "Список транспортных самолетов пуст!");
        for (MilitaryPlane militaryPlane : transportMilitaryPlanes) {
            Assert.assertEquals(militaryPlane.getType(), MilitaryType.TRANSPORT, "Найден не транспортный самолет!");
        }
    }
    ...
    public void testSortByMaxLoadCapacity() {
        ...
        for (int i = 0; i < planesSortedByMaxLoadCapacity.size() - 1; i++) {
            ...
            Assert.assertTrue(currentPlane.getMaxLoadCapacity() <= nextPlane.getMaxLoadCapacity(), 
                    "Сортировка по грузоподъемности нарушена!");
        }
    }
    ...
    public void testHasAtLeastOneBomberInMilitaryPlanes() {
        ...
        Assert.assertFalse(bomberMilitaryPlanes.isEmpty(), "Список бомбардировщиков пуст!");
        for (MilitaryPlane militaryPlane : bomberMilitaryPlanes) {
            Assert.assertEquals(militaryPlane.getType(), MilitaryType.BOMBER, "Найден не бомбардировщик!");
        }
    }
    ...
    public void testExperimentalPlanesHasClassificationLevelHigherThanUnclassified() {
        ...
        for (ExperimentalPlane experimentalPlane : experimentalPlanes) {
            Assert.assertNotEquals(experimentalPlane.getClassificationLevel(), ClassificationLevel.UNCLASSIFIED, 
                    "Найден секретный самолет с уровнем UNCLASSIFIED!");
        }
    }
}
